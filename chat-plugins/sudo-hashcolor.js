...
exports.commands = {
    ...
    hashcolor: function(target, room, user) {
        var name = toId(target);
        if (!target || name >= 19) return this.errorReply('You must include a username that is more than 0 and less than 19 characters');

        this.sendReply("|raw|<font size='5' color='" + color(name) + "'>" + name + "</font><br>The colour of this name is: <font color='" + color(name) + "'>" + color(name) + "</font>");
