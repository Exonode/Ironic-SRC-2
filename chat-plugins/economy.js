...
let shop = [
	...
	['Custom Symbol Color', 'Buys you the ability to have your symbol displayed as a different colour in up to 3 rooms', 15],
	['Custom Name Color', 'Buys you the ability to have your in-chat name color changed globally on this server', 70],
	...
	['Custom Emote', 'Buys you the ability to have a custom emote, you supply the image (Must be .jpg, .gif or .png) - Can be refused.', 50],
	['Room', 'Buys a chatroom for you to own. (within reason, can be refused)', 100],
	['Custom Command', 'This is basically a Custom Emote that only you can use', 100],
];
...
function currencyHeart(amount) {
	let name = " Heart";
	return amount <= 1 ? name : name + "s";
}
function currencySouls(amount) {
	let name = " Soul of the Non-Believers";
	return amount === 1 ? name : " Souls of the Non-Believers";
}
/**
...
function getShopDisplay(shop) {
	let display = "<div class='infobox infobox-limited'><table border='1' cellspacing='0' cellpadding='5' width='100%'>" +
	...
	display += "</tbody></table><center>To buy an item from the shop, use /buy <em>command</em>.</center></div>";
	return display;
}
...
exports.commands = {
	...
	hearts: function (target, room, user) {
		if (!this.runBroadcast()) return;
		if (!target) target = user.name;

		const amount = Db('money').get(toId(target), 0);
		if (amount === 0) return this.sendReplyBox(Tools.escapeHTML(target) + " has No " + currencyHeart(amount) + ".");
		if (amount === 1) return this.sendReplyBox(Tools.escapeHTML(target) + " has A " + currencyHeart(amount) + ".");
		else this.sendReplyBox(Tools.escapeHTML(target) + " has " + amount + currencyHeart(amount) + ".");
	},
	heartshelp: ["/hearts [user] - Shows the amount of money a user has."],

	souls: function (target, room, user) {
		if (!this.runBroadcast()) return;
		if (!target) target = user.name;

		const amount = Db('money').get(toId(target), 0);
		this.sendReplyBox(Tools.escapeHTML(target) + " has consumed " + amount + currencySouls(amount) + ".");
	},
	soulshelp: ["/hearts [user] - Shows the amount of money a user has."],
	...
	resetmoney: function (target, room, user) {
		...
		this.sendReply(target + " now has 0 Shekels.");
